...
def extract_pdf_text_with_bboxes(pdf_path: str):
    ...
    # df.to_excel("raw_pdf_text_with_bboxes.xlsx", index=False)
    print("✅ Saved bounding-box text → raw_pdf_text_with_bboxes.xlsx")
    return df
...
def extract_pdf_structured_from_text(pdf_path: str,
    ...
    print("✅ Saved structured table → structured_from_text.xlsx")
...
def sync_one_source_df(
    ...
    return total_changes, len(archive_rows_buffer)
...
def sync_sources_to_claims(
    ...
    changes_1, arc_1 = sync_one_source_df(
        src_df=src1_df,
    ...
    changes_2, arc_2 = sync_one_source_df(
        src_df=src2_df,
    ...
    changes_3, arc_3 = sync_one_source_df(
        src_df=source3_df,
    ...
    if source4_df is not None and not source4_df.empty:
        ...
        changes_4, arc_4 = sync_one_source_df(
            src_df=source4_df,
    ...
    print(f"Unmatched claims found: {len(unmatched_claims)}")
    ...
    def find_in_sources(claim_no):
        ...
        return {"Claim Number": claim_no_norm, "Carrier Paid": None, "Carrier Reserve": None,
    ...
    # ✅ SAVE OUTPUT EXCEL
    # ==========================================================
    out_month = datetime.now().strftime("%B").replace(" ", "_")
    out_year = datetime.now().strftime("%Y")
    out_filename = f"Claims_Not_Matched_{out_month}_{out_year}.xlsx"

    out_folder = r"C:\Users\IlaBarshilia\ACME Barricades\FDOT Web Scraping - FDOT Web Scraping Data\Claims Log Automation"
    out_path = os.path.join(out_folder, out_filename)

    df_unmatched.to_excel(out_path, index=False)

    print(f"\n✅ Created unmatched claims file:\n{out_path}")
    wb.save(CLAIMS_XLSX)
